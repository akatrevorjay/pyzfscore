...
class EnumMetaclass(type):
    ...
    def __call__(cls, *args, **kwargs):
        ...
        name, source = args
        return _make(cls, name, source, start=start)
...
class BitMaskEnumValue(int, EnumValue):
    """An EnumValue that is also an integer and converts it's value to bitmasks, ie 1=1, 2=2, 3=4, 4=8."""

    def __new__(cls, enum, value, attr):
        value = 1 << value
        return super(BitMaskEnumValue, cls).__new__(cls, value)
...
class BitMaskEnum(Enum):
    """A specialized enumeration with values that are also integer bitmasks."""
    __value_factory__ = BitMaskEnumValue
...
def _make(enum_class, name, source, start=1):
    ...
    try:
        ...
        source = enumerate(source, start=start)
